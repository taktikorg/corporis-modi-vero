# @taktikorg/corporis-modi-vero

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@taktikorg/corporis-modi-vero'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@taktikorg/corporis-modi-vero` preserves information of big numbers. `@taktikorg/corporis-modi-vero` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@taktikorg/corporis-modi-vero` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@taktikorg/corporis-modi-vero):

```
npm install @taktikorg/corporis-modi-vero
```
...
```js
import { parse } from '@taktikorg/corporis-modi-vero'
...
```js
import { parse, isInteger } from '@taktikorg/corporis-modi-vero'
...
```js
import { parse, isSafeNumber } from '@taktikorg/corporis-modi-vero'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@taktikorg/corporis-modi-vero'
...
(Spoiler: `@taktikorg/corporis-modi-vero` is much slower than native)
